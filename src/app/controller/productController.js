...
class ProductController {
  ...
  async getAllProduct(req, res) {
    try {
      var command =
        ...
        ", " +
        "GROUP_CONCAT(CONCAT(\"{id: \",ProductImage.id,\"\"), CONCAT(\", image: '\",ProductImage.image_path,\"'}\")) as image" +
        ", " +
        "Product.create_time" +
        ", " +
        "Product.update_time " +
  ...
  async addProductWithoutImage(req, res, next) {
    try {
      var {categoryID, userID, name, descriptions, price, quantity, discount, unitID} = req.body;
      var command =
